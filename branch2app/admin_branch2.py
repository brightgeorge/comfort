...
def branch1_room_create2(request):
...
def branch1_room_create_regi2(request):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            if chk_room == True:
                ...
                messages.info(request, 'BRANCH2 roon no already exists')
...
def pg1_bed_create_regi2(request):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            if chk_bd_code == True:
                ...
                context = {
                    'brname': 'BRANCH TWO Room Creation Form',
                    'br': pg1_new_beds.objects.all().order_by('roon_no'),
                    'brname': 'BRANCH TWO'
                }
                messages.info(request, 'BRANCH2 room created sucessfully')
...
def single_pg1_bed_create_regi2(request):
    if 'username' in request.session:
        if request.method == 'POST':
...
def pg1_view_all_beds2(request):
    if 'username' in request.session:
        context = {
            'brname': 'BRANCH two Room',
            'br': pg1_new_beds.objects.all().order_by('roon_no')
        }
        return render(request,'branches/branch2/beds/view_all_beds.html',context)
    return render(request,'index.html')

def delete_bed2(request,id):
    if 'username' in request.session:
...
def update_bed_basic_details2(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            room_type = request.POST.get('roomtype')
            ...
            return pg1_view_all_beds2(request)
        context = {
            'brname': 'BRANCH 2 Room Creation Form',
            'br': pg1_new_beds.objects.all().order_by('roon_no'),
            'brname': 'BRANCH 2',
