...
def branch1_room_create_ob_ch(request):
...
def pg1_bed_create_ob_ch(request):
    if 'username' in request.session:
...
def single_pg1_bed_create_regi_ob_ch(request):
    if 'username' in request.session:
        if request.method == 'POST':
...
def pg1_view_all_beds_ob_ch(request):
    if 'username' in request.session:
...
def update_bed_basic_details_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            return pg1_view_all_beds_ob_ch(request)
