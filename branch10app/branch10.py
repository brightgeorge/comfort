...
def branch1_dashboard_ob_ch(request):
    if 'username' in request.session:
        ...
        a = admin_dashboard_calculations_br10.grand_total_collection()
        ...
        context = {
            ...
            'name': us,
            'total_count_active_guests': admin_dashboard_calculations_br10.total_count_active_guests(),
            'total_count_vaccant_rooms': admin_dashboard_calculations_br10.total_count_vaccant_rooms(),
            'grand_total_collection': gtc,
            'total_collection_advance': admin_dashboard_calculations_br10.total_collection_advance(),
            'total_discount': admin_dashboard_calculations_br10.total_discount(),

            'total_colatable_amount': admin_dashboard_calculations_br10.total_colatable_amount(),
            'total_collected_amount': admin_dashboard_calculations_br10.total_collected_amount(),
            'total_due': admin_dashboard_calculations_br10.total_due(),
            'l': admin_dashboard_calculations_br10.grand_total(),
            #'total_collection_discount_june' : admin_dashboard_calculations_br10.total_collection_discount_june(),
            'y': admin_dashboard_calculations_br10.bar_chart(),
        }
...
def update_br1_admit_guest_ob_ch(request, id):
    if request.method == 'POST':
        selfmob = request.POST.get('selfmobno')
        # chk_mob = pg1_new_guest.objects.all().filter(self_mob=selfmob).exists()
        chk_mob = 10
        if chk_mob == 11:
            ...
            messages.info(request, 'BRANCH_ob_ch guest updated sucessfully')
            return view_all_new_guest_ob_ch(request)
...
def vacate_br1_guest_ob_ch(request, id):
    if request.method == 'POST':
        ...
        if chk_mob == 11:
            ...
            messages.info(request, 'BRAMCH_ob_ch guest already exists')
            # return render(request, 'branches/branch1/new_guest/view_all_new_guest.html', context)
            return view_all_new_guest_ob_ch(request)
        else:
            ...
            messages.info(request, 'BRANCH_ob_ch guest Vacated sucessfully')
            return view_all_new_guest_ob_ch(request)
...
def view_all_guest_ob_ch(request):
...
def shift_guest_regi_ob_ch(request):
...
def unpaid_rent_choose_months_ob_ch(request):
    if 'username' in request.session:
        us = request.session['username']
        bgs = background_color.objects.all().filter(username=us)
        bg = background_color.objects.all().filter(username=us).exists()
...
def details_of_unpaid_guests_ob_ch(request, id):
...
def paid_rent_choose_months_ob_ch(request):
    if 'username' in request.session:
        us = request.session['username']
        bgs = background_color.objects.all().filter(username=us)
        bg = background_color.objects.all().filter(username=us).exists()
...
def details_of_paid_guests_ob_ch(request, id):
    rno = pg1_new_guest.objects.all().filter(id=id)
    l = []
    for i in rno:
        l.append(str(i.roon_no))
...
def choose_months_ob_ch(request):
    if 'username' in request.session:
...
def choose_months_advance_ob_ch(request):
    if 'username' in request.session:
...
def jan_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def feb_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def march_advance_ob_ch(request):
    if 'username' in request.session:
        rn = request.POST.get('rno')
...
def march_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def april_advance_ob_ch(request):
    if 'username' in request.session:
        rn = request.POST.get('rno')
...
def april_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def may_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def june_advance_ob_ch(request):
    if 'username' in request.session:
        rn = request.POST.get('rno')
...
def june_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def july_advance_ob_ch(request):
    if 'username' in request.session:
        rn = request.POST.get('rno')
...
def july_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def auguest_advance_ob_ch(request):
    if 'username' in request.session:
        rn = request.POST.get('rno')
...
def auguest_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def sept_advance_ob_ch(request):
    if 'username' in request.session:
        rn = request.POST.get('rno')
...
def sept_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def october_advance_ob_ch(request):
    if 'username' in request.session:
        rn = request.POST.get('rno')
...
def october_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def nov_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def dec_make_payments_advance_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def jan_close_ob_ch(request):
    if 'username' in request.session:
        ...
        if chk == True:
            ...
            bc = branch_closing.objects.get(branch_name='branch_ob_ch')
            bc.jan = 1
            bc.save()
...
def jan_close_decision_page_ob_ch(request):
    if 'username' in request.session:
        chk = branch_closing.objects.all().filter(jan='', branch_name='branch_ob_ch').exists()
        print(chk)
        if chk == True:
...
def feb_close_decision_page_ob_ch(request):
    if 'username' in request.session:
        chk = branch_closing.objects.all().filter(feb='', branch_name='branch_ob_ch').exists()
        print(chk)
        if chk == True:
...
def mar_close_decision_page_ob_ch(request):
    if 'username' in request.session:
        chk = branch_closing.objects.all().filter(mar='', branch_name='branch_ob_ch').exists()
        print(chk)
        if chk == True:
...
def apr_close_decision_page_ob_ch(request):
    if 'username' in request.session:
        chk = branch_closing.objects.all().filter(apr='', branch_name='branch_ob_ch').exists()
        print(chk)
        if chk == True:
...
def may_close_decision_page_ob_ch(request):
    if 'username' in request.session:
        chk = branch_closing.objects.all().filter(may='', branch_name='branch_ob_ch').exists()
        print(chk)
        if chk == True:
...
def jun_close_decision_page_ob_ch(request):
    if 'username' in request.session:
        chk = branch_closing.objects.all().filter(jun='', branch_name='branch_ob_ch').exists()
        print(chk)
        if chk == True:
...
def jul_close_decision_page_ob_ch(request):
    if 'username' in request.session:
        chk = branch_closing.objects.all().filter(jul='', branch_name='branch_ob_ch').exists()
        print(chk)
        if chk == True:
...
def aug_close_decision_page_ob_ch(request):
    if 'username' in request.session:
        chk = branch_closing.objects.all().filter(aug='', branch_name='branch_ob_ch').exists()
        print(chk)
        if chk == True:
...
def sep_close_decision_page_ob_ch(request):
    if 'username' in request.session:
        chk = branch_closing.objects.all().filter(sep='', branch_name='branch_ob_ch').exists()
        print(chk)
        if chk == True:
...
def oct_close_decision_page_ob_ch(request):
    if 'username' in request.session:
        chk = branch_closing.objects.all().filter(oct='', branch_name='branch_ob_ch').exists()
        print(chk)
        if chk == True:
...
def nov_close_decision_page_ob_ch(request):
    if 'username' in request.session:
        chk = branch_closing.objects.all().filter(nov='', branch_name='branch_ob_ch').exists()
        print(chk)
        if chk == True:
...
def get_total_due_year_end_ob_ch(request):
    if 'username' in request.session:
        ag = pg1_new_beds.objects.all().order_by('roon_no')
        l = []
        t = []
        ...
        return render(request, 'branches/branch10/print_outs/get_total_due_year_end.html', context)
    return render(request, 'index.html')
...
def viewall_vacate_guest_ob_ch(request):
...
def jan_manke_payments_vacate_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            amt = request.POST.get('janamt')
            remark = request.POST.get('janremark')
...
def view_all_due_amt_ob_ch(request):
    us = request.session['username']
    bgs = background_color.objects.all().filter(username=us)
    bg = background_color.objects.all().filter(username=us).exists()
    a = []
...
def jan_account_mgt_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.jan_rent_flag = chk
            ic.save()
            return view_jan_account_details_ob_ch(request)
...
def feb_account_mgt_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.feb_rent_flag = chk
            ic.save()
            return view_feb_account_details_ob_ch(request)
...
def march_account_mgt_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.march_rent_flag = chk
            ic.save()
            return view_march_account_details_ob_ch(request)
...
def april_account_mgt_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.april_rent_flag = chk
            ic.save()
            return view_april_account_details_ob_ch(request)
...
def may_account_mgt_ob_ch(request, id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.may_rent_flag = chk
            ic.save()
            return view_may_account_details_ob_ch(request)
...
def june_account_mgt_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.june_rent_flag = chk
            ic.save()
            return view_june_account_details_ob_ch(request)
...
def july_account_mgt_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.july_rent_flag = chk
            ic.save()
            return view_july_account_details_ob_ch(request)
...
def auguest_account_mgt_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.auguest_rent_flag = chk
            ic.save()
            return view_auguest_account_details_ob_ch(request)
...
def sept_account_mgt_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.sept_rent_flag = chk
            ic.save()
            return view_sept_account_details_ob_ch(request)
...
def october_account_mgt_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.october_rent_flag = chk
            ic.save()
            return view_october_account_details_ob_ch(request)
...
def nov_account_mgt_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.nov_rent_flag = chk
            ic.save()
            return view_nov_account_details_ob_ch(request)
...
def dec_account_mgt_ob_ch(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            ic.dec_rent_flag = chk
            ic.save()
            return view_dec_account_details_ob_ch(request)
...
def manage_update_new_guest_ob_ch(request, id):
    if request.method == 'POST':
        ...
        if chk_mob == 11:
            ...
            messages.info(request, 'BRANCH_ob_ch guest already exists')
            # return render(request, 'branches/branch1/new_guest/view_all_new_guest.html', context)
            return manage_new_guest_ob_ch(request)
        else:
            name = request.POST.get('name')
...
def manage_update_beds_ob_ch(request, id):
    if request.method == 'POST':
        selfmob = request.POST.get('selfmobno')
        # chk_mob = pg1_new_guest.objects.all().filter(self_mob=selfmob).exists()
        chk_mob = 10
        if chk_mob == 11:
            ...
            messages.info(request, 'BRANCH_ob_ch guest updated sucessfully')
            return manage_bed_ob_ch(request)
