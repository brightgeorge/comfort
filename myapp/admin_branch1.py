...
def pg1_bed_create_regi(request):
    if 'username' in request.session:
        if request.method == 'POST':
...
def update_bed_basic_details(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            room_type = request.POST.get('roomtype')
            ...
            return pg1_view_all_beds(request)
