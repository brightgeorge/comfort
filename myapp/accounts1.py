...
from myapp.models import table1,in_exp_items_daily,opening_balance,category,ledger,accounts_book,background_color
...
def daily_category_wise(request):
    if 'username' in request.session:
...
def monthly_category_based_reports(request):
    if 'username' in request.session:
...
def yearly_category_based_reports(request):
    if 'username' in request.session:
...
def single_monthly_reports_choose_months(request):
    if 'username' in request.session:
...
def single_monthly_daily_in_exp_items_report(request,mo):
    if 'username' in request.session:
...
def accounts_dash_board(request):
    if 'username' in request.session:
