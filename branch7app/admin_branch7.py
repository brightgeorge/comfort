...
def branch1_room_create7(request):
...
def branch1_room_create_regi7(request):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            if chk_room == True:
                ...
                messages.info(request, 'BRANCH7 roon no already exists')
...
def pg1_bed_create_regi7(request):
    if 'username' in request.session:
        if request.method == 'POST':
...
def single_pg1_bed_create_regi7(request):
    if 'username' in request.session:
        if request.method == 'POST':
...
def pg1_view_all_beds7(request):
    if 'username' in request.session:
        ...
        context = {
            ...
            'brname': 'BRANCH 7 Room',
            'br': pg1_new_beds.objects.all().order_by('roon_no')
        }
        return render(request,'branches/branch7/beds/view_all_beds.html',context)
    return render(request,'index.html')

def delete_bed7(request,id):
    if 'username' in request.session:
...
def update_bed_basic_details7(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            room_type = request.POST.get('roomtype')
            ...
            return pg1_view_all_beds7(request)
