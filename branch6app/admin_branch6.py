...
def branch1_room_create6(request):
...
def branch1_room_create_regi6(request):
    if 'username' in request.session:
...
def pg1_bed_create6(request):
    if 'username' in request.session:
        ...
        context = {
            ...
            'brname':'BRANCH 6 Room Creation Form',
            'brname': 'BRANCH 6',
            'roomno' : room_pg1.objects.all().order_by('roon_no').values(),
        }
        return render(request,'branches/branch6/beds/create_beds.html',context)
    return render(request, 'index.html')

def pg1_bed_create_regi6(request):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            if chk_bd_code == True:
                ...
                context = {
                    ...
                    'br': pg1_new_beds.objects.all().order_by('roon_no').values(),
                    'brname': 'BRANCH 6'
                }
...
def single_pg1_bed_create_regi6(request):
    if 'username' in request.session:
        if request.method == 'POST':
...
def pg1_view_all_beds6(request):
    if 'username' in request.session:
        ...
        context = {
            ...
            'brname': 'BRANCH 6 Room',
            'br': pg1_new_beds.objects.all().order_by('roon_no').values(),
        }
        return render(request,'branches/branch6/beds/view_all_beds.html',context)
    return render(request,'index.html')

def delete_bed6(request,id):
    if 'username' in request.session:
...
def update_bed_basic_details6(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            room_type = request.POST.get('roomtype')
            ...
            return pg1_view_all_beds6(request)
