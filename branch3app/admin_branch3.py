...
def branch1_room_create_regi3(request):
    if 'username' in request.session:
        if request.method == 'POST':
            ...
            if chk_room == True:
                ...
                messages.info(request, 'BRANCH3 roon no already exists')
...
def pg1_bed_create_regi3(request):
    if 'username' in request.session:
        if request.method == 'POST':
...
def single_pg1_bed_create_regi3(request):
    if 'username' in request.session:
        if request.method == 'POST':
...
def update_bed_basic_details3(request,id):
    if 'username' in request.session:
        if request.method == 'POST':
            room_type = request.POST.get('roomtype')
